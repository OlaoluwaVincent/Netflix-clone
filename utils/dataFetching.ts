...
export const getFullDetailOfMovie: (
	props: number
) => Promise<DetailedMovie | undefined> = async (id) => {
	try {
...
export const getNowPlayingMovies: () => Promise<
	Movies[] | undefined
> = async () => {
	try {
		const res = await fetch(
...
export const getTopRatedMovie: () => Promise<
	Movies[] | undefined
> = async () => {
	try {
		const res = await fetch(
...
> = async () => {
	...
	try {
		const res = await fetch(
			'https://api.themoviedb.org/3/trending/all/week?api_key=f5ea505f0d7d67fe191c61ef531b8428'
		);
		const data = await res.json();
		return data.results.slice(0, 10);
	} catch (error) {
...
export const searchForMovies: (
	query: string
) => Promise<Movies[] | undefined> = async (query) => {
	// Region can be specified
	try {
		const res = await fetch(
			`https://api.themoviedb.org/3/search/multi?api_key=f5ea505f0d7d67fe191c61ef531b8428&language=en-US&query=${query}&page=1`
		);
		const data = await res.json();
		return data.results;
	} catch (error) {
...
export const MovieVideo: (
	movieId: number
) => Promise<Video | undefined> = async (movieId: number) => {
	// Region can be specified
	try {
		const res = await fetch(
			`https://api.themoviedb.org/3/movie/${movieId}/videos?api_key=f5ea505f0d7d67fe191c61ef531b8428&language=en-US`
		);
		const data = await res.json();
...
export const getLatestSeries: () => Promise<
	Movies[] | undefined
> = async () => {
	// Region can be specified
	try {
		const res = await fetch(
			'https://api.themoviedb.org/3/tv/popular?api_key=f5ea505f0d7d67fe191c61ef531b8428&language=en-US'
		);
		const data = await res.json();
		return data.results;
	} catch (error) {
